...
class Unet(nn.Module):
    def __init__(self, ms_ks, input_c):
        super().__init__()
        # ----------------- encoder part ----------------- 
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv1 = DoubleConv(input_c, 64)
    ...
    def forward(self, input, seg_gt=None, exist_gt=None):   
        # Complete the forward function for the rest of the encoder
        x1 = self.conv1(input)  #64
        x2 = self.max_pool(x1)
...
class Unet_SCNN(nn.Module):
    def __init__(
        ...
        self.unet1 = Unet(ms_ks, 3)
        self.unet2 = Unet(ms_ks, 64)
        self.unet3 = Unet(ms_ks, 64)
        self.unet4 = Unet(ms_ks, 64)
